...
@method_decorator([login_required], name='dispatch')
class EditMenuView(View): # название класса, он применяется в urls
    template_name = 'edit_menu.html' #ссылка на шаблон
    form = DishForm() #название формы

    def get_context(self): #начало функции
        context = {} #массив, который собирает в себя названия и представления, которые отправятся в шаблон
        context['form'] = self.form #в form записывается представление формы
        context['dishes'] = Dish.objects.all() #dishes - модель со всем меню
        ...
        context['sweet'] = Dish.objects.filter(category="SWE")
        context['bao'] = Dish.objects.filter(category="BAO")
        context['water'] = Dish.objects.filter(category="WAT")
        context['past'] = Dish.objects.filter(category="PST")
        context['pizza'] = Dish.objects.filter(category="PIZ")
        context['soupe'] = Dish.objects.filter(category="SOU")
        context['salad'] = Dish.objects.filter(category="SAL")
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if data.get('save'): #если нажата кнопка "save"
            dish = Dish.objects.get(id=data.get('save')) # dish - показываем с какой моделью работаем
            dish.category = data.get('category')# в колонку category записываем категорию, которую передали из шаблона
            dish.name = data.get('name')# в колонку name записываем название, которую передали из шаблона
            dish.description = data.get('description')#в колонку discription записываем описание, которую передали из шаблона
            dish.price = data.get('price')#в колонку price записываем стоимость, которую передали из шаблона
            dish.save()#сохраняем изменения в бд
        if data.get('delete'):#если нажата кнопка "delete"
            dish = Dish.objects.get(id=data.get('delete'))# dish - показываем с какой моделью работаем
            dish.delete()# удаляем всю строку из таблицы

        print(request.POST)
        context = self.get_context()
        return render(request, self.template_name, context) #то что возвращает функция request-обязательный парамметр
                                                            #self.template_name-ссылка на шаблона
                                                            #context - массив в параметрами, которые передаются в шаблон
